                    ...
                    if (key.equals(PushService.KEY_ACTION)) {
                        ...
                        if (action != null) {
                            if (action.equals(PushService.ACTIONS_SHOW_MESSAGE)) {
                                ...
                                String title = extras.getString(PushService.KEY_TITLE);
                                String body = extras.getString(PushService.KEY_BODY); // Получаем значение body
                                if (message != null) {
                                    Log.e(TAG, "MESSAGE_KEY  -> " + message);
                                    Log.e(TAG, "body_KEY  -> " + body);
                                    Log.e(TAG, "body_title  -> " + title);
                                    NotificationUtils.sendNotification(context, title, body); // Передаем значение body
